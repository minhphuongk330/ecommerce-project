...
const MainInfo: React.FC<MainInfoProps> = ({ product }) => {
	...
	return (
		<div className="w-full flex justify-center py-6 md:py-10 bg-white px-4 md:px-0">
			<div className="flex flex-col lg:flex-row gap-6 md:gap-[48px] w-full max-w-[1440px]">
				<ImageGallery images={imageUrls} productName={product.name} />
				<div className="w-full lg:w-[536px] flex flex-col">
					<h1 className="text-2xl md:text-[40px] font-bold text-black leading-tight mb-4 md:mb-[24px]">{product.name}</h1>
					<div className="flex items-end gap-3 md:gap-4 mb-4 md:mb-[24px]">
					...
					<SpecsGrid specs={product.specs} className="mb-4 md:mb-[24px]" />
					<p className="text-sm text-[#6F6F6F] leading-relaxed mb-4 md:mb-[24px]">
						{product.shortDescription}
						<span className="underline cursor-pointer text-black hover:opacity-70 ml-1">more...</span>
					</p>
